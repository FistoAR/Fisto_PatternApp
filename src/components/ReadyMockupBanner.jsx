...
export default function ReadyMockupBanner({
  ...
  return (
    ...
    >
      <div className={`frame-copy relative z-10 w-full max-w-[50%] lg:max-w-[700px] text-left ${compact ? 'lg:pl-[2%]' : ''}`}>
        <div className="frame-text mb-2 flex items-center gap-4">
        ...
        <h2
          className="frame-text mb-3 text-[clamp(18px,2.3vw,36px)] lg:text-[clamp(31px,2.7vw,38px)] font-bold leading-[1.28] text-white"
        >
          {renderedTitle}
        </h2>
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:flex-wrap md:gap-6 lg:gap-8">
          <p
            className="frame-text max-w-[280px] md:max-w-[340px] lg:max-w-[395px] text-[13px] lg:text-[15px] font-medium leading-[1.65] text-white shrink-0"
          >
            {subtitle}
          </p>
          {showButton && (
