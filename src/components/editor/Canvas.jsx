...
function drawUVs(mesh, components, faceColors, selectedFace, ctx, w, h, drawFull) {
...
function pointInPolygon(point, vs) {
  ...
  let x = point.u, y = point.v;
  let inside = false;
  for (let i = 0, j = vs.length - 1; i < vs.length; j = i++) {
    let xi = vs[i].u, yi = vs[i].v;
    let xj = vs[j].u, yj = vs[j].v;
...
export function extractUvComponents(mesh) {
  ...
  const parent = Array(triangles.length).fill(0).map((_, i) => i);
  const find = (i) => {
  ...
  for (const triIds of faceMap.values()) {
    ...
    triIds.forEach(triId => {
      ...
      const checkEdge = (a, b) => {
        ...
        let isBoundary = true;
        if (edge.tris.length >= 2) {
          const otherTriId = edge.tris[0] === triId ? edge.tris[1] : edge.tris[0];
...
const Canvas = forwardRef(({ textureCanvasRef, onTextureUpdated, modelUrl, showUv, fullUv, bgColor, isActive }, ref) => {
  ...
  const redrawDisplay = useCallback(() => {
    ...
    if (showUv && currentMeshRef.current) {
      drawUVs(currentMeshRef.current, uvComponents, faceColors, selectedFace, ctx, w, h, fullUv);
    }
  ...
  }, [showUv, fullUv, faceColors, selectedFace, uvComponents]);
  ...
  const bakeTexture = useCallback(() => {
    ...
    uvComponents.forEach(comp => {
      const color = faceColors[comp.id];
      if (color && comp.path && comp.path.length > 0) {
        bakeCtx.beginPath();
        bakeCtx.moveTo(comp.path[0].u * bakeCanvas.width, comp.path[0].v * bakeCanvas.height);
        for (let i = 1; i < comp.path.length; i++) {
          bakeCtx.lineTo(comp.path[i].u * bakeCanvas.width, comp.path[i].v * bakeCanvas.height);
        }
        bakeCtx.closePath();
    ...
    if (selectedFace) {
      const comp = uvComponents.find(c => c.id === selectedFace);
      if (comp && comp.path && comp.path.length > 0) {
        bakeCtx.save();
        bakeCtx.beginPath();
        bakeCtx.moveTo(comp.path[0].u * bakeCanvas.width, comp.path[0].v * bakeCanvas.height);
  ...
  }, [bgColor, faceColors, uvComponents, selectedFace, textureCanvasRef]);
  ...
  const restoreHistoryState = useCallback((state) => {
    if (!state) return;
  ...
  useEffect(() => {
    redrawDisplay();
  }, [showUv, fullUv, selectedFace, faceColors, uvComponents, redrawDisplay]);
  ...
  const applyResize = (img, mode, mx, my, scale, interaction) => {
    const startCx = (interaction.startImgX + interaction.startImgW / 2) / scale;
    const startCy = (interaction.startImgY + interaction.startImgH / 2) / scale;
    ...
    if (mode >= HANDLE.TL && mode <= HANDLE.BL) {
      ...
      let ax, ay, vx, vy;
      if (mode === HANDLE.TL) { ax = startLocalW/2; ay = startLocalH/2; vx = -startLocalW; vy = -startLocalH; }
      else if (mode === HANDLE.TR) { ax = -startLocalW/2; ay = startLocalH/2; vx = startLocalW; vy = -startLocalH; }
      else if (mode === HANDLE.BR) { ax = -startLocalW/2; ay = -startLocalH/2; vx = startLocalW; vy = startLocalH; }
      else if (mode === HANDLE.BL) { ax = startLocalW/2; ay = -startLocalH/2; vx = -startLocalW; vy = startLocalH; }
  ...
  const handleContextMenu = (e) => {
    ...
    if (clickedImage) {
      ...
      setContextMenu({ open: true, x: e.clientX - containerRect.left, y: e.clientY - containerRect.top, mode: 'image' });
    } else {
      const containerRect = containerRef.current.getBoundingClientRect();
      setContextMenu({ open: true, x: e.clientX - containerRect.left, y: e.clientY - containerRect.top, mode: 'canvas' });
    }
  };
  ...
  const getSelectedToolbarStyle = () => {
    ...
    const left = (canvasRect.left - containerRect.left) + imgX_unscaled * renderScale;
    const top = (canvasRect.top - containerRect.top) + imgY_unscaled * renderScale;
  ...
  const getSelectedFaceStyle = () => {
    ...
    if (!comp || !displayCanvasRef.current || !containerRef.current) return {};
    ...
    const cx_unscaled = selectedFaceUv ? selectedFaceUv.u * unscaledW : ((comp.minU + comp.maxU) / 2) * unscaledW;
    let top_unscaled = selectedFaceUv ? selectedFaceUv.v * unscaledH - (30 / zoom) : (comp.minV) * unscaledH - (15 / zoom);

    const left = (canvasRect.left - containerRect.left) + cx_unscaled * renderScale;
    const top = (canvasRect.top - containerRect.top) + top_unscaled * renderScale;
    ...
    <div className="flex-1 flex flex-col relative" style={{ background: '#f5efe6' }}>
      <div 
        ...
        onPointerDown={(e) => {
          ...
          if (e.target === e.currentTarget || e.target.id === 'canvas-bg-grid' || e.target.id === 'canvas-wrapper') {
            setSelectedFace(null);
            setSelectedFaceUv(null);
            setSelectedImage(null);
            selectedImageRef.current = null;
            if (contextMenu.open) setContextMenu({ ...contextMenu, open: false });
            redrawDisplay();
          }
        }}
      >
        ...
        <div id="canvas-wrapper" className="absolute inset-0 flex items-center justify-center p-4">
          <div className="relative">
            <canvas
              ...
              style={{ 
                cursor: toolMode === 'hand' ? (interactionRef.current.isPanning ? 'grabbing' : 'grab') : 'default',
                ...
                <div className="bg-white shadow-[0_4px_20px_rgba(0,0,0,0.15)] px-3 py-2 rounded-xl border border-gray-100 flex items-center gap-2 relative">
                  ...
                  {(() => {
                    ...
                    if (comp) {
                      ...
                      return <span className="absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 shadow-sm">&lt;--- {w} mm ---&gt;</span>;
                    }
                    return null;
                  })()}

                  <span className="text-[11px] font-bold text-gray-700 whitespace-nowrap">Face color</span>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 bg-white shadow-[0_4px_25px_rgba(0,0,0,0.15)] rounded-full px-4 py-2.5 flex items-center gap-2 border border-gray-100">
            ...
            <Tooltip label="Select Tool">
              ...
              >
                <img src={cursorIcon} alt="Cursor" className="w-6 h-6 object-contain opacity-80 hover:opacity-100" />
              </button>
            </Tooltip>
            <Tooltip label="Pan Tool">
              ...
              >
                <img src={handIcon} alt="Hand" className="w-6 h-6 object-contain opacity-80 hover:opacity-100" />
              </button>
            </Tooltip>
            ...
            {contextMenu.mode === 'image' && (
              <>
                    ...
                    <div className="bg-white shadow-xl rounded-lg border border-gray-100 min-w-[150px] py-1">
                      <ContextMenuItem onClick={onBringToFront}>
                        <div className="flex items-center gap-2">
                          <img src={frontIcon} className="w-4 h-4 opacity-70" alt="" />
                      ...
                      <ContextMenuItem onClick={onSendBackward}>
                        <div className="flex items-center gap-2">
                          <img src={sendIcon} className="w-4 h-4 opacity-70" alt="" />
                ...
                <Tooltip label="Duplicate" position="top">
                  <button onClick={onDuplicate} className="w-10 h-10 rounded-lg flex items-center justify-center border-none bg-transparent hover:bg-gray-100 cursor-pointer text-gray-700 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                ...
                <Tooltip label="Delete" position="top">
                  <button onClick={onDelete} className="w-10 h-10 rounded-lg flex items-center justify-center border-none bg-transparent hover:bg-gray-100 cursor-pointer text-gray-700 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 pb-2 hidden group-hover/menu:block">
                    <div className="bg-white shadow-xl rounded-lg border border-gray-100 min-w-[150px] py-1">
                      <ContextMenuItem onClick={() => {
                        ...
                        selectedImageRef.current.locked = !selectedImageRef.current.locked;
                        setSelectedImage(null);
                        selectedImageRef.current = null;
                        needsDisplayRedrawRef.current = true;
                        bakeTexture();
                        ...
                        <div className="flex items-center gap-2">
                          <img src={lockIcon} className="w-4 h-4 opacity-70" alt="" />
                          <span>{selectedImageRef.current?.locked ? 'Unlock' : 'Lock'}</span>
                        </div>
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => {
                        if(!selectedImageRef.current) return;
                        selectedImageRef.current.opacity = selectedImageRef.current.opacity === 1 ? 0.5 : 1;
                        needsDisplayRedrawRef.current = true;
                        bakeTexture();
                        saveState();
                        redrawDisplay();
                        ...
                        <div className="flex items-center gap-2">
                          <img src={transparentIcon} className="w-4 h-4 opacity-70" alt="" />
                          <span>Toggle Transparency</span>
                        </div>
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => {
                        if(!selectedImageRef.current) return;
                        selectedImageRef.current.flipX = !selectedImageRef.current.flipX;
                        needsDisplayRedrawRef.current = true;
                        bakeTexture();
                        saveState();
                        redrawDisplay();
                        ...
                        <div className="flex items-center gap-2">
                          <img src={flipHorizontalIcon} className="w-4 h-4 opacity-70" alt="" />
                          <span>Flip horizontal</span>
                        </div>
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => {
                        if(!selectedImageRef.current) return;
                        selectedImageRef.current.flipY = !selectedImageRef.current.flipY;
                        needsDisplayRedrawRef.current = true;
                        bakeTexture();
                        saveState();
                        redrawDisplay();
                        ...
                        <div className="flex items-center gap-2">
                          <img src={flipVerticalIcon} className="w-4 h-4 opacity-70" alt="" />
...
function Tooltip({ label, children, position = 'top' }) {
  ...
  return (
    <div className="relative group flex items-center justify-center">
      {children}
      <div className={`absolute ${posClasses[position]} px-2.5 py-1 bg-gray-900 text-white text-[11px] font-medium rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-[100] shadow-sm`}>
        {label}
        {/* Tooltip arrow */}
        <div className={`absolute left-1/2 -translate-x-1/2 w-0 h-0 border-solid border-4 border-transparent ${position === 'top' ? 'top-full border-t-gray-900' : 'bottom-full border-b-gray-900'}`}></div>
      </div>
    </div>
  );
}
