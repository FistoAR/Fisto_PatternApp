...
class DraggableImage {
  ...
  clone(offset = 32) {
    ...
    copy.tapeDetails = this.tapeDetails;
...
function uvToPx(u, v, w, h, isTape, offsetX = 0, offsetY = 0) {
  if (isTape) {
    return { x: v * w + offsetX, y: (1 - u) * h + offsetY };
  }
...
function pxToUv(px, py, w, h, isTape, offsetX = 0, offsetY = 0) {
  ...
  if (isTape) {
    return { u: 1 - localY / h, v: localX / w };
  }
  return { u: localX / w, v: localY / h };
}

// Offscreen canvas cache for static UV grid wireframe
let uvCacheCanvas = null;
let uvCacheKey = null;
...
function drawUVs(
  ...
  // 2. Stroke wireframe or outlines as gray dashed (cached for performance)
  const shouldDrawFull =
    !isTapeModel && (drawFull || !components || components.length === 0);

  const currentKey = `${mesh.uuid}_${w}_${h}_${shouldDrawFull}_${offsetX}_${offsetY}`;

  if (uvCacheKey !== currentKey || !uvCacheCanvas) {
    if (!uvCacheCanvas) {
    ...
    cacheCtx.strokeStyle = "#9ca3af"; // gray-400
    ...
    if (shouldDrawFull) {
      ...
      if (isTapeModel && uvTapeMerged) {
        ...
        Object.values(mergedGroups).forEach((group) => {
          ...
          cacheCtx.beginPath();
          cacheCtx.moveTo(tl.x, tl.y);
          cacheCtx.lineTo(tr.x, tr.y);
          cacheCtx.lineTo(br.x, br.y);
          cacheCtx.lineTo(bl.x, bl.y);
          cacheCtx.closePath();
          cacheCtx.stroke();
        });
    ...
    cacheCtx.restore();
    uvCacheKey = currentKey;
  }

  ctx.save();
  ctx.drawImage(uvCacheCanvas, 0, 0);
  ctx.restore();
  ...
  (
    ...
    const resizeTextureCanvas = useCallback(
      (nextSize, forTapeModel = false) => {
        ...
        if (
          bakeCanvas &&
          (bakeCanvas.width !== rawW || bakeCanvas.height !== rawH)
        ) {
          bakeCanvas.width = rawW;
          bakeCanvas.height = rawH;
        }
      },
      [textureCanvasRef],
    );
    ...
    const bakeTexture = useCallback(
      (ignoreSelection = false) => {
        ...
        bakeCtx.save();
        if (!isTapeBake) {
          // flip Y so UV origin (bottom-left in GL) maps to top-left in canvas
          bakeCtx.translate(0, bakeCanvas.height);
          bakeCtx.scale(1, -1);
        } else {
          // Horizontal layout to vertical UV map rotate.
          bakeCtx.transform(0, -1, -1, 0, bakeCanvas.width, bakeCanvas.height);
        }
    ...
    const restoreHistoryState = useCallback(
      (state) => {
        ...
        const newImages = state.images.map((itemData) => {
          ...
          di.tapeDetails = itemData.tapeDetails;
    ...
    useEffect(() => {
      ...
      loader.load(modelUrl, (gltf) => {
        ...
        if (modelUrl && modelUrl.includes("Tape")) {
          console.log("Extracted UV Components for Tape:", components);
        }
        ...
        if (modelUrl && modelUrl.includes("Tape")) {
          ...
          // Identify "strips" that wrap around the tape (large V range)
          const strips = [];
          components.forEach((comp) => {
            if (!comp.loops || comp.loops.length === 0) return;
          ...
          // Sort strips by minU
          strips.sort((a, b) => a.minU - b.minU);

          // Cluster adjacent strips in U
          let clusterIndex = 0;
          ...
          strips.forEach((comp) => {
            if (!currentCluster) {
              ...
              // If adjacent or overlapping in U (with a small tolerance for rounding)
              if (comp.minU <= currentCluster.maxU + 0.05) {
                ...
                mergedGroups[`band_${clusterIndex++}`] = currentCluster;
                currentCluster = {
                  minU: comp.minU,
                  maxU: comp.maxU,
                  minV: comp.minV,
        ...
        for (let i = imagesRef.current.length - 1; i >= 0; i--) {
          ...
          if (
            img.contains(
              ...
              offsetYRef.current,
              uvComponentsRef.current,
              !!(modelUrl && modelUrl.includes("Tape")),
    ...
    function onUploadImage(fileOrUrl, fitType = null, tapeDetails = null) {
      if (!fileOrUrl) return;
      const url =
      ...
      img.onload = () => {
        const handleImageReady = (loadedImg) => {
          ...
          newImg.tapeDetails = tapeDetails;
          ...
          if (hasSelected) {
            ...
            if (fitType === "cover") {
              if (isTapeModel && tapeDetails && tapeDetails.layoutHeight && tapeDetails.totalCanvasHeight) {
                const scaleFactorY = tapeDetails.totalCanvasHeight / tapeDetails.layoutHeight;
                imgWidth = widthInTex;
                imgHeight = heightInTex * scaleFactorY;
              } else {
                imgWidth = widthInTex;
    ...
    useImperativeHandle(ref, () => ({
      ...
      uploadImage: (file, fitType = null, tapeDetails = null) => {
        onUploadImage(file, fitType, tapeDetails);
      },
      ...
      applyFitToSelectedImage: (fitType) => {
        ...
        if (hasSelected && fitType) {
          ...
          let imgWidth, imgHeight;
          if (fitType === "cover") {
            if (isTapeModel && sel.tapeDetails && sel.tapeDetails.layoutHeight && sel.tapeDetails.totalCanvasHeight) {
              const scaleFactorY = sel.tapeDetails.totalCanvasHeight / sel.tapeDetails.layoutHeight;
              imgWidth = widthInTex;
              imgHeight = heightInTex * scaleFactorY;
            } else {
              imgWidth = widthInTex;
