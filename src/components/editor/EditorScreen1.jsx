  ...
  useEffect(() => {
    ...
    clonedScene.traverse((obj) => {
      if (obj.isMesh && !obj.userData.isDecal && obj.material) {
        const nameLower = (obj.name || "").toLowerCase();
        const mats = Array.isArray(obj.material) ? obj.material : [obj.material];
        const isCapOrLid = mats.some((m) => {
          if (!m) return false;
          const matName = (m.name || "").toLowerCase();
          return (
            matName.includes("cap") ||
  ...
  useEffect(() => {
    ...
    clonedScene.traverse((obj) => {
      ...
      mArray.forEach((m) => {
        ...
        const isCapMat = id.toLowerCase().includes("cap") || id.toLowerCase().includes("lid") || id.toLowerCase().includes("circle003");
        let colorHex = null;
        if (isCapMat && appliedColors) {
          const capKey = Object.keys(appliedColors).find(
            (k) =>
              k.toLowerCase().includes("cap") ||
        ...
        if (
          ...
          const tryInherit = (appliedObj) => {
            ...
            if (isLidLabel && !isBodyLabel) return lidKey ? appliedObj[lidKey] : null;
            if (isBodyLabel && !isLidLabel) return bodyKey ? appliedObj[bodyKey] : null;
            ...
            const loadMap = (url, mapType, isColorSpace) => {
              ...
              textureCache
                .load(url, {
                  ...
                  flipY: false,
                  colorSpace: isColorSpace ? THREE.SRGBColorSpace : undefined,
                })
                .then((texture) => {
                  const imageAspect =
...
export default function EditorScreen1({
  ...
  return (
    <div className="flex flex-col h-full w-full relative">
        ...
        {isSwappingModel && (
          <div style={{
            ...
            <div style={{
              ...
              height: '36px',
              borderRadius: '50%',
              border: '3.5px solid rgba(192, 98, 58, 0.2)',
              borderTopColor: '#c0623a',
            ...
            `}</style>
            <span style={{ marginTop: 10, fontSize: '12px', fontWeight: 700, color: '#c0623a' }}>Loading Model...</span>
          </div>
        )}
        <R3FCanvas
      ...
      {false && activeTab === "edit" && isBottleModel && (
        <div
          ref={capPanelRef}
