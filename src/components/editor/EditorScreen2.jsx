...
export default function EditorScreen2({
  ...
  const [pendingTapeLayoutDataUrl, setPendingTapeLayoutDataUrl] =
    useState(null);
  const [pendingTapeLayoutInfo, setPendingTapeLayoutInfo] = useState(null);
  const [showConfirmBackDialog, setShowConfirmBackDialog] = useState(false);
  ...
  const handleBackAttempt = () => {
    const isDirty = canvasRef.current?.isDirty?.() ?? false;
    if (isDirty) {
      setShowConfirmBackDialog(true);
    } else {
      onBack();
    }
  };
        ...
        >
          ...
          {showTapeLayout && (
            <TapeLayoutScreen
              ...
              onSave={(dataUrl, layoutInfo) => {
                setShowTapeLayout(false);
                if (dataUrl) {
                  setPendingTapeLayoutDataUrl(dataUrl);
                  setPendingTapeLayoutInfo(layoutInfo);
                }
              }}
            />
          )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#111827' }}>Discard Changes?</h3>
              <p style={{ margin: 0, fontSize: '13px', color: '#6b7280', lineHeight: '1.5' }}>
                You have unsaved edits on your canvas. Going back now will discard all changes.
              </p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%', marginTop: '8px' }}>
              <button
                onClick={() => setShowConfirmBackDialog(false)}
                style={{
                  width: '100%',
                  padding: '12px',
                  background: '#c0623a',
                ...
                onMouseEnter={e => e.currentTarget.style.background = '#a65330'}
                onMouseLeave={e => e.currentTarget.style.background = '#c0623a'}
              >
                Keep Editing
              </button>
              <button
                onClick={() => {
                  setShowConfirmBackDialog(false);
                  onBack();
                }}
                style={{
                  width: '100%',
